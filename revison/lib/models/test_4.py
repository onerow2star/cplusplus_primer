...
class PoseHighResolutionNet(nn.Module):
    ...
    def __init__(self, cfg, **kwargs):
        ...
        self.point_layer_3 = nn.Conv2d(
            in_channels=pre_stage_channels[0],
            # out_channels=16,  # 测试 mpii
            out_channels=17,  # 测试 coco
            kernel_size=1,
            stride=1,
            padding=0
        )
        ...
        self.sap = SpatialAttention(output_chl_num=cfg['MODEL']['NUM_JOINTS'])
        self.ca = ChannelAttention(output_chl_num=2)

        self.ca1 = ChannelAttention(output_chl_num=32)

        self.c1_conv1x1 = nn.Conv2d(32, 32, 1, 1)
        self.c1_param_adapter = nn.Sequential(
            # nn.Conv2d(2, 2, 3, 2),
            # nn.MaxPool2d(kernel_size=2, stride=2), # new
            nn.Conv2d(32, 32, 3, 2),
            nn.MaxPool2d(kernel_size=2, stride=2),
            ...
            nn.MaxPool2d(kernel_size=2, stride=2),
            nn.Conv2d(32, 32, 3, padding=1))
        self.c1_refine_layer = conv_bn_relu(32, 32, kernel_size=1,
                                                  stride=1, padding=0, has_bn=True, has_relu=True)
        self.c2_conv1x1 = nn.Conv2d(64, 64, 1, 1)
        self.c2_param_adapter = nn.Sequential(
            # nn.MaxPool2d(kernel_size=2, stride=2), # new
            nn.Conv2d(64, 64, 3, 1),
            nn.MaxPool2d(kernel_size=2, stride=2),
            ...
            nn.MaxPool2d(kernel_size=2, stride=2),
            nn.Conv2d(64, 64, 3, padding=1))
        self.c2_refine_layer = conv_bn_relu(64, 64, kernel_size=1,
                                            stride=1, padding=0, has_bn=True, has_relu=True)
    ...
    def forward(self, x):
        ...
        # ****************
        # batchsize = x.size(0)
        # adaptive_conv_c1 = AdaptiveConv2d(batchsize * 32,
        #                                   batchsize * 32,
        #                                   7, padding=3,
        #                                   groups=batchsize * 32,
        #                                   bias=False)
        # 
        # k_res0 = self.c1_conv1x1(k)  # 1
        # p_p1 = self.c1_param_adapter(p)
        # k_res0 = adaptive_conv_c1(k_res0, p_p1)
        # k_res0 = self.c1_refine_layer(k_res0)
        # k_res0 = self.ca1(k_res0)
        # k = (k_res0 + k)
        # *************
        ...
        # jia kuo hao ming xian ti sheng
        fk1 = fk1_1 # ra
        fk2 = fk2_1 + fk2_2# rk
        fk3 = fk3_1 + fk3_2 # rh
        fk4 = fk4_1 + fk4_2
